...
export default {
  ...
  async asyncData({ $axios, query, $auth }) {
    ...
    // afdeling_id
    const afdeling_list = await $axios.$get(`/api/admin/lov_afdeling_hpt`)
    ...
    if (query.q_afdeling_id) {
      // console.log('rendra')
      $axios
        .get(`/api/admin/lov_afdeling_hpt?afdeling_id=${q_afdeling_id}`)
        .then((response) => {
    ...
    const posts = await $axios.$get(
      `/api/admin/report/activity_actual_hpt?q=${search}&page=${page}&activitied_at_prepend=${activitied_at_start}&activitied_at_append=${activitied_at_end}&q_foreman_employee_id=${q_foreman_employee_id}&q_afdeling_id=${q_afdeling_id}`
    )
